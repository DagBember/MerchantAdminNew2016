...
<body style='background-color:rgb(247,247,247);margin:0px' ">
...
<script>
    ...
    function level_2_report_4(period) {
        if (ClientCall(arguments)) {
            ...
            var hideStatus = ajax.get_server_value("hide_status");
    ...
    function select_chain(sChain) {
        if (ClientCall(arguments)) {
            ajax.call_server("select_chain()",sChain);
        }
        else if (ServerCall(arguments)) {
            ajax.render_server_html();
            var e = document.getElementById("chain_dialog");
            e.style.display="";
        }
    }

    function select_chain_hide()
    {
        if (ClientCall(arguments)) {
            ajax.call_server("select_chain_hide()");
        }
        else if (ServerCall(arguments)) {
            ajax.render_server_html();
        }
    ...
    function select_chains_available()
    {
        if (ClientCall(arguments)) {
            ajax.call_server("select_chains_available()");
        }
        else if (ServerCall(arguments)) {
            ajax.render_server_html();
        }
    ...
    function bember_working(bStatus)
    {
        var bemberWorkingDiv = document.getElementById("bember_working");

        if (!bemberWorkingDiv) return;

        if (bStatus == true)
        {
            bemberWorkingDiv.innerHTML = "Working";

            var boxes = document.getElementsByClassName('bember_month_box');
            if (boxes)
            {
            ...
            var boxes2 = document.getElementsByClassName('bember_month_box_toggled');
            if (boxes2)
            {
            ...
            {
            bemberWorkingDiv.innerHTML = "Ready";
