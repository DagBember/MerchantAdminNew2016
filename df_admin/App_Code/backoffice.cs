using System; // Liten kommentar 12. januar
...
public class backoffice : DATABASE_ACCESS
{
    ...
    public class SHOP_ITEM
    {
        public string sId;
        public string sName;
        public SHOP_ITEM(string _sId, string _sName)
        {
            sId = _sId;
            sName = _sName;
        }
    ...
    public List<SHOP_ITEM> getSelectListOfChains(string sParentShop)
    {

        List<SHOP_ITEM> shopList = new List<SHOP_ITEM>();
        ...
        try
        {
            string sSql = "select s.name, s.id from shop s where parent_shop=" + sParentShop;
            ...
            while (reader.Read())
            {
                shopList.Add(new SHOP_ITEM(reader.c("id").ToString(), reader.c("name").ToString()));
            }
        } catch (Exception e)
        {
        } finally
