...
public class ChainStandardReport : CHAIN_REPORT
{
    ...
    public ChainStandardReport(Global _global)
	{
        global = _global;
        sChainId = global.accessController.getChainID();
	}
    ...
    public string getReport_B_basket_sums(String sHeading, int iYear, DateTime startDatePeriod, DateTime endDatePeriod, STANDARD_REPORT_TYPE reportType)
    {
        ...
        List<backoffice.chain_shop_standard_report_item> chainShopList = global.www_backoffice().get_chain_shop_total_list(sChainId, thisYearStart, thisYearEnd);

        if (reportType == STANDARD_REPORT_TYPE.D_BASKET_COUNT)
            chainShopList = global.www_backoffice().fill_baskets_per_member_report(sChainId, chainShopList);


        List<backoffice.chain_shop_invoice> chainBasketList = global.www_backoffice().get_chain_shop_BASKET_list( sChainId, thisYearStart, thisYearEnd, startDatePeriod,endDatePeriod);
    ...
    public string A_StandardReport(string sHeading, Global global, DateTime startDate, DateTime endDate)
    {
        ...
        */
        ...
        // SQL Here
        string sChainId = global.accessController.getChainID();
