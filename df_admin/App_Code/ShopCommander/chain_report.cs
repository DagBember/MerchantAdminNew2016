...
public class CHAIN_REPORT : SHOP_BASE
{
    ...
    public static bool event_catched_and_performed(xAjax ajax, Global global)
    {
        ...
        else if (sProcedure == "level_2_report_2()")
        {
            ...
            ChainStandardReport csr = new ChainStandardReport(global);
        ...
        else if (sProcedure == "chain_get_dashboard_data()")
        {
            ...
            if (iIndex == 0)
            {
                if (bMaximized == false)
                {
                    List<backoffice.dim_value> monthList = global.www_backoffice().dash_mini_month_member_old(DateTime.Now.Year.ToString(),global.accessController.getChainID());
        ...
        else if (sProcedure == "level_2_report_4()")
        {
            ...
            if (global.accessController.isSuperUser())
                ajax.WriteVariable("hide_status", "false");
            else
                ajax.WriteVariable("hide_status", "true");

            WRITE_DASHBOARD_TO_WORK_PAGE(iDashboardPeriod, global,ajax);
        }
