...
public class DASHBOARD_NEW
{
    ...
    public string get_BEMBER_CHAIN_Dashboard(DASHBOARD_PERIOD currentDashboardPeriod, List<backoffice.name_value_value_value_value> conversionList,List<backoffice.shop_top_bottom> topBottomShops)
    {
        ...
        List<backoffice.dim_value_value> shopList = global.www_backoffice().dash_get_members_by_chain_shop(global.accessController.getChainID(), currentDashboardPeriod); // 14 okt = 65
        List<backoffice.dim_value_value> shopListAllPeriods = global.www_backoffice().dash_get_members_by_chain_shop(global.accessController.getChainID(), DASHBOARD_PERIOD.DONT_CARE); // 5 okt nå




        sb.Append("<div style='width:100%;'>"); // denne funker !!!
        sb.Append("<table align=left width=100% cellpadding=0 cellspacing=0>");
        sb.Append("<tr>");
        ...
        sb.Append("<div style='width:100%;'>"); // 7 jan 
    ...
    private string A_GetHorisontalGreenTop(DASHBOARD_PERIOD currentDashboardPeriod, List<backoffice.dim_value_value> shopList, List<backoffice.dim_value_value> shopListAllPeriods)
    {
        ...
        // 14 okt = 68
        List<backoffice.dim_dim_value> yearMonthList = global.www_backoffice().dash_maxi_month_member((int)DateTime.Now.Year, global.accessController.getChainID());

        List<backoffice.dim_dim_value_value> yearMonthBasketSumList = global.www_backoffice().dash_get_average_member_basket((int)DateTime.Now.Year, global.accessController.getChainID());


        List<backoffice.dim_value> sexList = global.www_backoffice().get_consumer_sex_list(global.accessController.getChainID(), currentDashboardPeriod);

        string sAverageAge = global.www_backoffice().get_average_member_age(global.accessController.getChainID(), currentDashboardPeriod);

        sb.Append("<td align=center style='background-color:rgb(37, 126, 98)'>");
        sb.Append("<div>");
    ...
    private string append_row_44()
    {
        ...
        List <backoffice.dim_value> shopList = global.www_backoffice().get_monthly_invoice(global.accessController.getChainID());
...
public class DASHBOARD_GREEN
{
    ...
    public string A_1_GetGreenPart(Global global,DASHBOARD_PERIOD currentPeriod, List<backoffice.dim_dim_value> yearMonthList, List<backoffice.dim_value_value> shopList)
    {
        ...
        // 14 okt = 68
        sb.Append(XXX_getCenteredGreenFrame(0, 10, 300, 220, 170, "MEDLEMSBASE", "TOTALT ANTALL MEDLEMMER", "I PERIODEN", "ENDRING I %", iTotalMembers.ToString("### ### ##0"),
            global.getSvgUpDownArrow("&nbsp;" + iInPeriod.ToString("### ### ##0"), true),
    ...
    public string A_3_GreenPart(Global global, DASHBOARD_PERIOD currentPeriod, List<backoffice.dim_dim_value_value> yearMonthList)
    {
        ...
        string sPercentChange = get_percent_increase_from_to(dInPrevPeriod, dInPeriod, true);
        sb.Append(A_1_GreenPart("frame_2", 0, 10, 300, 220, 170, "GJENNOMSNITTSKJØP " + new DASHBOARD_MONTH(currentPeriod).sThisMonth, "FORRIGE MND.", "ENDRING I %", dInPeriod.ToString() + " kr", dInPrevPeriod.ToString("### ### ##0") + " kr", global.getSvgUpDownArrow("&nbsp;" + sPercentChange + "&nbsp;%",false)));
        return sb.ToString();
    }
