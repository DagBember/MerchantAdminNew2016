...
public class webservice_common : System.Web.Services.WebService
{
    ...
    public ShopParentChild getShopParentChildFrom(webservice_database databaseParent, string sShopId)
    {
        ShopParentChild parentChild = new ShopParentChild();

        GLOBAL_SQL_CONN conn = new GLOBAL_SQL_CONN(databaseParent);

        try
        {
            string sSql = "select id, parent_shop from shop where id=" + sShopId;
            ...
            if (reader.Read())
            {
                parentChild.sId = reader.c("id").ToString().Trim();
                parentChild.sParentId = reader.c("parent_shop").ToString().Trim();
                if (isBlank(parentChild.sParentId))
                    parentChild.bOnTop = true;
